...
def load_data():
    ...
    return df, id2word, word2id, word2gold_jsd, source_token2vecs, target_token2vecs
...
def cross_validate_method(method_name, metric_func, df, id2word, word2gold_jsd, 
    ...
    for trial in range(n_trials):
        ...
        valid_gold_scores = [word2gold_jsd[id2word[valid_id]] for valid_id in valid_ids]
        
        test_ids = [test_id for test_id in range(len(df)) if test_id not in valid_ids]
        test_gold_scores = [word2gold_jsd[id2word[test_id]] for test_id in test_ids]
...
def save_results(results, output_dir):
    """Save results to table3.txt"""
    output_path = Path(output_dir)
    output_path.mkdir(exist_ok=True)
    
    table_path = output_path / "table3.txt"
    ...
    for method_name in methods:
        try:
            ...
            result = cross_validate_method(
                method_name, metric_func, df, id2word, word2gold_jsd,
